...
// 获取最近7天
  ...
  getThisWeek2() {
    ...
    let today = date.getDay();
    //上周日距离今天的天数（负数表示）
    let stepSunDay = -today;
    //  如果今天是周日
    if (today == 0) {
      stepSunDay = -7;
    }
    //周一距离今天的天数（负数表示）
    let stepMonday = stepSunDay - 6;
    let time = date.getTime();
    let monday = new Date(time + stepMonday * 24 * 3600 * 1000);
    let sunday = new Date(time + stepSunDay * 24 * 3600 * 1000);
    // 本周一的日期 （起始日期）
    let startDate = this.transferDate(monday, true); // 日期变换
    // 本周日的日期 （结束日期）
    let endDate = this.transferDate(sunday, false); // 日期变换
  ...
  getMonth(num) {
    ...
    let sunday = new Date(year + '/' + month + '/' + 31);
    if(sunday.getDate() == 1){
      sunday = new Date(year + '/' + month + '/' + 30);
    }
    // 月初 （起始日期）
    let startDate = this.transferDate(monday, true); // 日期变换
    // 月末 （结束日期）
  ...
  getHMS() {
    ...
    if(minu<10 && sec<10) {
      var nowTime = hour +":0"+minu +":0"+ sec
    }
    else if(minu<10) {
      var nowTime = hour +":0"+minu +":"+ sec
    }
    else if(sec<10) {
      var nowTime = hour +":"+minu +":0"+ sec
    }
    else {
